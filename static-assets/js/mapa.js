...
function inicializarEventos(el, cfg) {
    ...
    el.inputArquivo.addEventListener('change', async (e) => {
        ...
        const continuar = await confirmarSubstituicao();
...
async function confirmarSubstituicao() {
    ...
    try {
        ...
        const response = await fetch(
            window.location.pathname.replace(/\/$/, '') + "/verificar/"
        );
        ...
        if (data.existe) {
            ...
            return confirm(
                "Já existe um mapa carregado.\n\n" +
                "Ao enviar um novo arquivo, o mapa atual será substituído.\n\n" +
                "Deseja continuar?"
            );
